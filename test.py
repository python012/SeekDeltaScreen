...
REQUIRE_GENERATE_NEW_SCREENLIB = False
...
def initializeOldScreenlib():
  ...
  for i in range(1, len(OLD_SCREEN_FILE_DIR) + 1):
    ...
    for f in cur.fetchall():
      ...
      conn.close()
    conn.close()
    print "Analysing old screens " + str(i) + " / " + str(len(OLD_SCREEN_FILE_DIR))
...
def initializeNewScreenlib():
  ...
  for i in range(1, len(NEW_SCREEN_FILE_DIR)+1):
    ...
    print "Analysing new screens " + str(i) + " / " + str(len(NEW_SCREEN_FILE_DIR))
...
def seekDeltaScreen():
  ...
  delta_screen_page << h1('Missing Screen List', cl='center')
